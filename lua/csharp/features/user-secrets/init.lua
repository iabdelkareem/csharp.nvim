...
-- doesn't exists
local function _ensure_secret_exists(user_secret_file_path)
  local file, _ = io.open(user_secret_file_path, "r+")

  if not file then
    os.execute("mkdir -p " .. user_secret_file_path)
    os.execute("chmod 600 " .. user_secret_file_path)
  end
end
...
--- @return string | nil
local function _extract_user_id_from_command(output_init_command)
...
local function _extract_user_id_from_project(project_path)
  ...
  if not project_file then
    logger.error("Something went wrong during read the project file", { feature = "user-secrets" })
    return
  end
...
local function _init_secret(project_path)
  ...
  if output then
    return _extract_user_id_from_command(output)
  end
end

local function _open_secret_in_buffer(user_secret_id)
  local secret_file_path = os.getenv("HOME") .. "/.microsoft/usersecrets/" .. user_secret_id .. "/secrets.json"
  _ensure_secret_exists(secret_file_path)

  vim.cmd.edit(secret_file_path)
end
...
local function _open_secret()
  ...
  if not project_information then
    logger.error("No project selected", { feature = "user-secrets" })
    return
  end
  ...
  if not user_secret_id then
    ...
    if not user_secret_id then
      logger.error("Something went wrong during user secrets creation", { feature = "user-secrets" })
      return
    end
  end
